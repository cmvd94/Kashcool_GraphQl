...
export const addSubject = async (req: CustomRequest, res: Response) => {
  ...
  if (!subjectName || !subjectDescription || !req.file) {
    return res
      .status(400)
      .json({ message: "Subject name, description, and image are required" });
  }
  ...
  try {
    ...
    if (existingSubject) {
      return res
        ...
        .json({
          message:
            "Subject with this name already exists in the selected region and grade",
        });
    }
    ...
    const fileName = `${region}/${gradeName}/${semesterName}/${subjectName}/${
      req.file.originalname.split(".")[0]
    }${uuidv4()}.${fileExtension}`; // Full file path
    ...
    const semester = gradeDoc.semesters?.find(
    ...
    await UpdateAdminDataEntryLog.findOneAndUpdate(
      { region: region, grade: grade, semester: semesterName }, // Find this combination
      { $set: { updatedAt: new Date() } }, // Update the "updatedAt" field
      { upsert: true, new: true } // Options: create new if it doesn't exist, return the updated version
    );
...
export const editSubject = async (req: CustomRequest, res: Response) => {
  ...
  if (!subjectName || !subjectDescription) {
    return res
      .status(400)
      .json({ message: "Subject name and description are required" });
  }
  ...
  try {
    ...
    if (existingSubject) {
      return res
        ...
        .json({
          message:
            "Another subject with this name already exists in the selected region and grade",
        });
    }
    ...
    if (req.file) {
      ...
      const fileExtension = req.file.mimetype.split("/")[1];
      fileName = `${region}/${gradeName}/${semesterName}/${subjectName}/${
        req.file.originalname.split(".")[0]
    ...
    await logAdminAction(
      ...
      "Subject",
      subject._id.toString(),
      `Edited subject: ${subjectName}`
    );

    // Update changes in UpdateDataEntryLog
    await UpdateAdminDataEntryLog.findOneAndUpdate(
      { region: region, grade: grade, semester: semesterName }, // Find this combination
      { $set: { updatedAt: new Date() } }, // Update the "updatedAt" field
      { upsert: true, new: true } // Options: create new if it doesn't exist, return the updated version
    ...
    if (err instanceof Error) {
      ...
      return res
        .status(500)
        .json({ message: "Error editing subject", error: err.message });
    }
    return res.status(500).json({ message: "Unknown error occurred" });
  }
};
...
export const deleteSubjectController = async (
  ...
  try {
    ...
    await deleteSubject(
      ...
      grade.toString(),
      semesterName
    ); // Pass ObjectId and additional parameters
    ...
    await logAdminAction(
      ...
      "Subject",
      subjectId,
      `Deleted subject with ID: ${subjectId}`
    );
    // Update changes in UpdateDataEntryLog
    await UpdateAdminDataEntryLog.findOneAndUpdate(
      { region: region, grade: grade, semester: semesterName }, // Find this combination
