...
export const createLesson = async (req: CustomRequest, res: Response) => {
  ...
  const { region, grade } = req.user; // Extract region from user
  ...
  try {
    ...
    // subcription key.  when user subscribed same key is generated and sent .
    const subkey = region.toString() + grade.toString() + sub.semester;
    // Create new lesson
    ...
    await UpdateAdminDataEntryLog.findOneAndUpdate(
      { region: region, grade: grade, semester: sub.semester }, // Find this combination
...
export const deleteLessonController = async (
  ...
  try {
    ...
    // Update changes in UpdateDataEntryLog
    await UpdateAdminDataEntryLog.findOneAndUpdate(
      { region: region, grade: grade, semester: subject?.semester }, // Find this combination
      { $set: { updatedAt: new Date() } }, // Update the "updatedAt" field
      { upsert: true, new: true } // Options: create new if it doesn't exist, return the updated version
    );
